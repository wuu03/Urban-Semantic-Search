...
def get_embedding(text):
    """
    Generate vector embedding for the input processed_text using the loaded model.
...
def split_text(text, limit=CHUNK_SIZE_LIMIT):
    """
    Splits processed_text into chunks respecting sentence boundaries (. ; ! ?).
...
def build_lean_meta_1808(texts_list, people_list, geo_props_list):
    ...
    # Iterate through processed_text records
    for t in texts_list:
...
def build_lean_meta_1740(row):
    ...
    Retains fields specifically requested for processed_text presentation and filtering.
    """
...
def generate_detailed_body_1808(t_item):
    """
    Converts a single raw processed_text entry into a fluent descriptive paragraph.
...
def process_1808_integrated(json_path):
    ...
    for idx, entry in enumerate(data):
        texts_list = entry.get('processed_text', [])
        ...
        # --- E. Row Generation ---
        # If processed_text is split into chunks, we create multiple rows sharing the SAME MultiPolygon.
    ...
    output_file = '../data/processed_text/processed_1808.parquet'
...
def process_1740_integrated(tsv_path, geojson_path, napo_lookup):
    ...
    if os.path.exists(geojson_path):
        ...
        if 'uid' in gdf_geo.columns:
            ...
            # Merge: Right join on processed_text means we keep all processed_text records.
            merged_df = gdf_geo.merge(df_text, on='uid', how='right', suffixes=('_geo', ''))
        else:
            print("Error: 'uid' not found in GeoJSON. Proceeding with processed_text only.")
            merged_df = df_text
            merged_df['geometry'] = None
    else:
        print("GeoJSON not found. Proceeding with processed_text only.")
        merged_df = df_text
        merged_df['geometry'] = None
    ...
    for idx, row in merged_df.iterrows():
        ...
        for i, chunk in enumerate(chunks):
            # For 1740, the "chunk" is often just the whole processed_text, but if split,
            ...
            processed_rows.append({
                ...
                'original_id': uid,
                'chunk_id': f"{uid}_{i}",
                'text_representation': final_text,  # The chunked processed_text
                'embedding': get_embedding(final_text),
    ...
    # Save to Parquet
    df = pd.DataFrame(processed_rows)
    output_file = '../data/processed_text/processed_1740.parquet'
    df.to_parquet(output_file)
